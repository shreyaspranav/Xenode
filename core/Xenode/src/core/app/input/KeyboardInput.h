...
namespace Xen {
	...
	enum class KeyAction : uint8_t { Release = 0, Press = 1, Hold = 2 };
	...
	class XEN_API KeyboardInput
	{
		friend class DesktopGameApplication;
	public:
		// Get the status of the Key
		static bool IsKeyPressed(KeyboardKeyCode code);

		// These set of methods should only be called by children of GameApplication
	private:
		// Sets up all the events that can be raised by keyboard and sets the "active window" to window
		static void Init(const Ref<Window>& window);
	};
}
// These are required for representing the enums as strings and the macro _TO_STRING_IMPL in Events.h
